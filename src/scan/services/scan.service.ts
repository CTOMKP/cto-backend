...
export class ScanService {
  ...
  async scanToken(contractAddress: string, userId?: number) {
    try {
      if (!contractAddress) {
    ...
    try {
      ...
      const scanPromises = validAddresses.map(async (contractAddress) => {
        try {
          ...
          let eligible = false;
          if (error.message?.includes('Token not found') || error.message?.includes('account not found')) errorMessage = 'Token not found. Please verify the contract address is correct.';
          else if (error.message?.includes('API') || error.message?.includes('timeout')) errorMessage = 'External API temporarily unavailable.';
          else if (error.message?.includes('Invalid') || error.message?.includes('format')) errorMessage = 'Invalid contract address format.';
          return { contractAddress, success: false, error: errorMessage, eligible, details: process.env.NODE_ENV === 'development' ? error.message : undefined };
        }
      });
