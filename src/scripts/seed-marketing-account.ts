...
async function main(){
 ...
 for(let i=0;i<amounts.length;i++){const eventKey=`${marker}-earning-${i+1}`,u=referred[i],data={creatorAccountId:account.id,creatorUserId:user.id,referredUserId:u.id,sourceType:types[i%4],sourceId:`${marker}-source-${i+1}`,amountGross:amounts[i]*5,platformFeeAmount:amounts[i],creatorCutPercent:20,amountEarned:amounts[i],status:(i<10?'PAID':'AVAILABLE') as 'PAID'|'AVAILABLE',eventKey,createdAt:ago(28-i),metadata:{marker,synthetic:true}}; const old=await db.creatorEarning.findFirst({where:{eventKey}}); old?await db.creatorEarning.update({where:{id:old.id},data}):await db.creatorEarning.create({data}); await db.creatorReferral.update({where:{referredUserId:u.id},data:{totalEarned:amounts[i]}});}
