 ...
 * @param {number} ageInDays - Age in days (can be fractional)
 * @returns {string} Formatted age string
 */
export function formatTokenAge(ageInDays) {
  if (ageInDays < 1) {
 ...
 * @param {number} ageInDays - Age in days (can be fractional)
 * @returns {string} Short formatted age string
 */
export function formatTokenAgeShort(ageInDays) {
  if (ageInDays < 1) {
    // For tokens less than 1 day old, show hours/minutes
    const hours = Math.floor(ageInDays * 24);
    if (hours === 0) {
